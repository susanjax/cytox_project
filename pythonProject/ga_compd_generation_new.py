...
population_size = 100
test_compound =pd.read_csv('test_generation.csv')
df = pd.read_csv('v2/data/original_preprocessed.csv')
...
def population(size):
  ...
  new = new_one.reset_index(drop=True)
  # new = neww.iloc[:size,:]
...
def cell_lines(dat):
  ...
  single_norm = uniq_cell_data.loc[df['cell line'] == 'HepG2'] # cho-k1 cell line
  single_canc = uniq_cell_data.loc[df['cell line'] == 'L-02'] # skov-3 cell line
